...
const resolvers = {
  Query: {
    getRecipes: async (root, args, req, info) => {
      const { query, userId, ingredients, matchAll, difficulty, first = 12, after } = args;
      console.log(args);
      ...
      if (ingredients && ingredients.length > 0) {
        pipeline.unshift({
          $match: {
            'ingredients.name': matchAll ? { $all: ingredients } : { $in: ingredients }
          }
        });
      }
      ...
      const recipes = await Recipe.aggregate(pipeline).sort(sort).limit(first).exec();
    ...
    getComments: async (root, args, req, info) => {
      ...
      return await Comment.find({ recipe: id }).populate('user', '-password', User).exec();
    },

    getAllIngredients: async (root, args, req, info) => {
      try {
        const pipeline = [{ $unwind: '$ingredients' }, { $group: { _id: '$ingredients.name' } }, { $sort: { _id: 1 } }];
    ...
    getRecipe: async (root, args, req, info) => {
      try {
        ...
        const recipe = await Recipe.findById(id).populate('user', '-password', User).exec();
